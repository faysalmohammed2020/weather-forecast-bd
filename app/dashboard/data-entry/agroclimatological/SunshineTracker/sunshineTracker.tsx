...
export default function SunshineTracker() {
  ...
  const handleHourChange = (index: number, value: string) => {
    ...
    // Don't accept values > 100
    if (numValue > 100) {
      return;
    }

    // Convert to 0-1 range by dividing by 100
    newHours[index] = numValue / 100;
            ...
            <Card className="shadow-lg border-0 bg-white/80 backdrop-blur">
              <CardHeader className="bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-t-lg p-3">
                ...
                <CardDescription className="text-blue-100">
                  Enter sunshine duration (0-100 scale, where 100 = 1 hour)
                </CardDescription>
              </CardHeader>
                    ...
                    {timeSlots.map((slot, index) => (
                      <div key={index} className="space-y-2">
                        ...
                        <Input
                          ...
                          value={
                            currentHours[index]
                              ? Math.round(currentHours[index] * 100).toString()
                              : ""
                          }
                          onChange={(e) =>
                        ...
                        <div className="text-xs text-center text-gray-500">
                          {currentHours[index] > 0
                            ? `${currentHours[index].toFixed(2)}h`
                            : ""}
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-green-50 p-4 rounded-lg">
                      ...
                      <p className="text-2xl font-bold text-green-800">
                        {monthlyStats.totalSunshine.toFixed(2)}h
                      </p>
                    </div>
                    <div className="bg-yellow-50 p-4 rounded-lg">
                      ...
                      <p className="text-2xl font-bold text-yellow-800">
                        {monthlyStats.averageDaily.toFixed(2)}h
                      </p>
                    </div>
